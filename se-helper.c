...
{
	if (sizeof(result) == ECDH_KEY_BYTE_LENGTH
	    || sizeof(source) == ECDH_KEY_BYTE_LENGTH)
		goto error;
...
int ecdh_calc(uint8_t *shared_secret, uint8_t *peer_pubkey, uint8_t *keyID)
{
	...
	sw_status = Se05x_API_CheckObjectExists(&pSession->s_ctx, key, &result);
	if (SM_OK != sw_status) {
	...
	if (reverse_key_endianness(tempkey, peer_pubkey))
		goto failurelv1;

	sw_status = Se05x_API_ECDHGenerateSharedSecret(&pSession->s_ctx,	//pSe05xSession_t session_ctx,  //session_ctx Session Context [0:kSE05x_pSession]
	...
	if (reverse_key_endianness(shared_secret, tempkey2))
		goto failurelv1;
...
int get_se_key(uint8_t *outkey, uint8_t *keyID)
{
	...
	status = sss_key_object_get_handle(&keypair, key);
	if (status != kStatus_SSS_Success) {
		sss_key_object_free(&keypair);
		sss_key_store_context_free(&pCtx.ks);
		ex_sss_session_close(&pCtx);
	...
	status =
	    sss_key_store_get_key(&pCtx.ks, &keypair, derbuf, &dersz,
				  &derszbits);
	if (status != kStatus_SSS_Success) {
		sss_key_object_free(&keypair);
		sss_key_store_context_free(&pCtx.ks);
